...
const NotificationCard = ({ data, index, notificationState }) => {
    ...
    let { seen, type, reply, createdAt ,comment, user, user: { personal_info: { profile_img, fullname, username } }, replied_on_comment, blog: { _id, blog_id, title }, _id: notification_id } = data
        ...
        <div className={"p-6 border-b border-grey border-l-black " + (!seen ? "border-l-2" : "")}>
            <div className="flex gap-5 mb-3">
                ...
                <div className="w-full">
                    ...
                    </h1>
                    {
                        ...
                        :
                        <Link className="font-medium text-dark-grey hover:underline line-clamp-1" to={`/blogs/${blog_id}`}>{`"${title}"`}</Link>
                    }
                </div>
            </div>
            {
...

            {
                ...
                <div className="ml-20 p-5 bg-grey mt-5 rounded-md">
                    ...
                    <button onClick={(e) => handleDelete(comment._id, "reply", e.target)} className="underline hover:text-black ml-14 mt-4">Delete</button>
                </div> :"" 
            }
