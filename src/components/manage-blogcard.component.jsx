    ...
    return(
        <>
            <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
                ...
                <img src={banner} className="max-md:hidden lg:hidden xl:block w-28 h-28 flex-none object-cover" />
    ...
    return(
        <div className="flex gap-5 lg:gap-10 pb-6 border-b mb-6 border-grey">
            ...
            <div>
                ...
                <div className="flex gap-6 mt-3">
                    <Link to={`/editor/${blog_id}`} className="underline pr-4 py-2">Edit</Link>
...
const deleteBlog = (blog, access_token, target) => {
    ...
    .then(({ data }) => {
        ...
        // Update the parent component state to reflect the deleted blog
        setStateFunc(prevState => {
            const { deletedDocCount, totalDocs, results } = prevState;

            // Remove the deleted blog from the results
            const updatedResults = results.filter((_, i) => i !== index);

            // Update the document counts accordingly
            const updatedState = {
                ...prevState,
